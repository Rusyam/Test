...
from instructions import txt_instruction, txt_sits, txt_test1, txt_test2, txt_test3
...
class PulseScr(Screen):
    def __init__(self, **kwargs):
        ...
        self.btn = Button(test = 'Начать', size_hint = (0.3 , 0.4), pos_hint = {'senter_x':0.5})
        self.btn.on_press = self.next
        outer = BoxLayout(orientation = 'vertical', padding = 8, spacing = 8)
        outer.add_widget(instr)
...
class CheckSits(Screen):
    def __init__(self, **kwargs):
        ...
        self.btn = Button(text = 'Продолжить',size_hint = (0.3, 0.2), poz_hint = {'center_x': 0.5})
        self.btn.on_press = self.next
        outer = BoxLayout(orientation = 'vertical', padding = 8, spacing = 8)
        outer.add_widget(instr)
        outer.add_widget(self.btn)
...
class PulseScr2(Screen):
    def __init__(self, **kwargs):
        ...
        lbl =_result1 = Label(text = 'Результат:', halign = 'right')
        ...
        outer.add_widget(self.lbl1_sec)
        outer.add_widget(line1)
        outer.add_widget(line2)
        outer.add_widget(self.btn)
        self.add_widget(outer)
...
class HeartCheck(App):
    def build(self):
        sm = ScreenManager()
        sm.add_widget(IntstScr(name = 'instr'))
